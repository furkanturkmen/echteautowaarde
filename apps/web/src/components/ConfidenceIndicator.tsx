...
export function ConfidenceIndicator({
  ...
  /** "row" spreads the meter and its explanation across a full-width strip. */
  layout?: "stack" | "row";
}) {
  ...
  return (
    <div className={layout === "row" ? "grid gap-x-10 gap-y-4 lg:grid-cols-[22rem_1fr]" : ""}>
      <div>
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
          ...
          <span className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-muted sm:text-sm">{strength}</span>
          </span>
        </div>
      ...
      <div>
        <p className={`text-sm text-muted ${layout === "row" ? "" : "mt-2.5"}`}>
          Dit is een maat voor de bewijskracht: hoeveel en hoe goed vergelijkbare advertenties
          we hebben gevonden.
          {weakest ? (
            <>
              {" "}
